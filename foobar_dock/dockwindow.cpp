...
void DockWindow::OnContextMenu(HWND hWnd, CPoint point)
{
	...
	enum {
		...
		//ID_CURRENT_PLAYLIST_LAST = ID_CURRENT_PLAYLIST_FIRST + 999,

		//ID_PLAYLIST_FIRST,
		//ID_PLAYLIST_LAST = ID_PLAYLIST_FIRST + 1000,

		ID_CURRENT_PLAYLIST_SHOW,
		ID_CURRENT_PLAYLIST_REMOVE_DEAD,
		ID_CURRENT_PLAYLIST_REMOVE_DUPLICATES,
		ID_CURRENT_PLAYLIST_DESTROY,
		ID_CURRENT_PLAYLIST_ADD_FILES,
		ID_CURRENT_PLAYLIST_ADD_FOLDER,

		ID_ACTIVE_PLAYLIST_PLAY,
		ID_ACTIVE_PLAYLIST_SHOW,
		ID_ACTIVE_PLAYLIST_REMOVE_DEAD,
		ID_ACTIVE_PLAYLIST_REMOVE_DUPLICATES,
		ID_ACTIVE_PLAYLIST_DESTROY,
		ID_ACTIVE_PLAYLIST_ADD_FILES,
		ID_ACTIVE_PLAYLIST_ADD_FOLDER,

		ID_PLAYLIST_PLAY_RANDOM,
		ID_PLAYLIST_CREATE_FILES,
		ID_PLAYLIST_CREATE_FOLDER,

		ID_LISTED_PLAYLIST_PLAY,
		ID_LISTED_PLAYLIST_PLAY_LAST = ID_LISTED_PLAYLIST_PLAY + 999,
		ID_LISTED_PLAYLIST_SHOW,
		ID_LISTED_PLAYLIST_SHOW_LAST = ID_LISTED_PLAYLIST_SHOW + 999,
		ID_LISTED_PLAYLIST_REMOVE_DEAD,
		ID_LISTED_PLAYLIST_REMOVE_DEAD_LAST = ID_LISTED_PLAYLIST_REMOVE_DEAD + 999,
		ID_LISTED_PLAYLIST_REMOVE_DUPLICATES,
		ID_LISTED_PLAYLIST_REMOVE_DUPLICATES_LAST = ID_LISTED_PLAYLIST_REMOVE_DUPLICATES + 999,
		ID_LISTED_PLAYLIST_DESTROY,
		ID_LISTED_PLAYLIST_DESTROY_LAST = ID_LISTED_PLAYLIST_DESTROY + 999,
		ID_LISTED_PLAYLIST_ADD_FILES,
		ID_LISTED_PLAYLIST_ADD_FILES_LAST = ID_LISTED_PLAYLIST_ADD_FILES + 999,
		ID_LISTED_PLAYLIST_ADD_FOLDER,
		ID_LISTED_PLAYLIST_ADD_FOLDER_LAST = ID_LISTED_PLAYLIST_ADD_FOLDER + 999,
		...
		ID_CONTEXT_FIRST,
		ID_CONTEXT_LAST = ID_CONTEXT_FIRST + 999,
	...
	int flag, playlistNum;
	pfc::string_formatter text;
	pfc::string8 format;
	std::wstring wstr;
	bool anythingPlaying = false;

	int currentPlaylistNum = pm->get_playing_playlist();
	int activePlaylistNum = pm->get_active_playlist();
	...
	if (pc->is_playing())
	{
		...
		//our options
		AppendMenu(hSubMenuCurrentPlaylist, MF_STRING, ID_CURRENT_PLAYLIST_SHOW, TEXT("Show"));
		AppendMenu(hSubMenuCurrentPlaylist, MF_SEPARATOR, 0, 0);
		AppendMenu(hSubMenuCurrentPlaylist, MF_STRING, ID_CURRENT_PLAYLIST_REMOVE_DEAD, TEXT("Remove Dead Entries"));
		AppendMenu(hSubMenuCurrentPlaylist, MF_STRING, ID_CURRENT_PLAYLIST_REMOVE_DUPLICATES, TEXT("Remove Duplicates"));
		AppendMenu(hSubMenuCurrentPlaylist, MF_STRING, ID_CURRENT_PLAYLIST_DESTROY, TEXT("Destroy Playlist"));
		AppendMenu(hSubMenuCurrentPlaylist, MF_SEPARATOR, 0, 0);
		AppendMenu(hSubMenuCurrentPlaylist, MF_STRING, ID_CURRENT_PLAYLIST_ADD_FILES, TEXT("Add Files..."));
		AppendMenu(hSubMenuCurrentPlaylist, MF_STRING, ID_CURRENT_PLAYLIST_ADD_FOLDER, TEXT("Add Folder..."));
		...
		// Which playlist is currently playing
		pm->playlist_get_name(currentPlaylistNum, text);
	...
	// "Selected Playlist"
	if ((pc->is_playing() && currentPlaylistNum != activePlaylistNum) || !pc->is_playing())
	{		
		cmm->init_context_playlist(0);
		...
		//our options
		AppendMenu(hSubMenuActivePlaylist, MF_STRING, ID_ACTIVE_PLAYLIST_PLAY, TEXT("Play"));
		...
		AppendMenu(hSubMenuActivePlaylist, MF_SEPARATOR, 0, 0);
		AppendMenu(hSubMenuActivePlaylist, MF_STRING, ID_ACTIVE_PLAYLIST_ADD_FILES, TEXT("Add Files..."));
		AppendMenu(hSubMenuActivePlaylist, MF_STRING, ID_ACTIVE_PLAYLIST_ADD_FOLDER, TEXT("Add Folder..."));
		...
		/*if (cmm->get_root())
		{
			...
			//and also foobar's options
			cmm->win32_build_menu(hSubMenuActivePlaylist, ID_ACTIVE_PLAYLIST_FIRST, ID_ACTIVE_PLAYLIST_LAST);
		}*/
		
		// Which playlist is active
		pm->playlist_get_name(activePlaylistNum, text);

		wstr = L"Selected Playlist: ";
		wstr += UtfConverter::FromUtf8(std::string(text.get_ptr()));
	
		AppendMenu(hMenu, MF_POPUP, (UINT_PTR)hSubMenuActivePlaylist, wstr.c_str());
	}
	...
	// "Playlist"
	int playlistsNumber = pm->get_playlist_count();
	...
	if (playlistsNumber)
	{
		...
		for (int i = 0; i < playlistsNumber; i++)
		{
			...
			if (i != activePlaylistNum)
				AppendMenu(hSubMenuActivePlaylist, MF_STRING, ID_LISTED_PLAYLIST_PLAY + i, TEXT("Play"));

			AppendMenu(hSubMenuActivePlaylist, MF_STRING, ID_LISTED_PLAYLIST_SHOW + i, TEXT("Show"));
			AppendMenu(hSubMenuActivePlaylist, MF_SEPARATOR, 0, 0);
			AppendMenu(hSubMenuActivePlaylist, MF_STRING, ID_LISTED_PLAYLIST_REMOVE_DEAD + i, TEXT("Remove Dead Entries"));
			AppendMenu(hSubMenuActivePlaylist, MF_STRING, ID_LISTED_PLAYLIST_REMOVE_DUPLICATES + i, TEXT("Remove Duplicates"));
			AppendMenu(hSubMenuActivePlaylist, MF_STRING, ID_LISTED_PLAYLIST_DESTROY + i, TEXT("Destroy Playlist"));
			AppendMenu(hSubMenuActivePlaylist, MF_SEPARATOR, 0, 0);
			AppendMenu(hSubMenuActivePlaylist, MF_STRING, ID_LISTED_PLAYLIST_ADD_FILES + i, TEXT("Add Files..."));
			AppendMenu(hSubMenuActivePlaylist, MF_STRING, ID_LISTED_PLAYLIST_ADD_FOLDER + i, TEXT("Add Folder..."));
		...
		AppendMenu(hSubMenuPlaylistsList, MF_STRING, ID_PLAYLIST_PLAY_RANDOM, TEXT("Play Random :)"));
	...
	// "Preferences"
	if (cfg_show_preferences_option)
	...
	// "Playlist"->"Play Random"
	else if (cmd == ID_PLAYLIST_PLAY_RANDOM)
	{
		int randomNumber = static_api_ptr_t<genrand_service>()->genrand(playlistsNumber - 1);
		cmd = ID_LISTED_PLAYLIST_PLAY + randomNumber;
	}

	// "Playlist"->"Play" or "Selected Playlist"->"Player"
	if (cmd >= ID_LISTED_PLAYLIST_PLAY && cmd <= ID_LISTED_PLAYLIST_PLAY_LAST
		|| cmd == ID_ACTIVE_PLAYLIST_PLAY)
	{
		playlistNum = (cmd == ID_ACTIVE_PLAYLIST_PLAY ? activePlaylistNum : cmd - ID_LISTED_PLAYLIST_PLAY);
		pm->set_active_playlist(playlistNum);

		// Autoplay
		if (advconfig_switched_playlist_changes_song.get())
		{
			pm->playlist_execute_default_action(playlistNum, 0);
	...
	else if (cmd == ID_PLAYLIST_CREATE_FILES)
	{
		...
		if (advconfig_playlist_undo_backup.get())
			pm->playlist_undo_backup(activePlaylistNum);


		// Save it to not minimize when it was activated
		bool minimize = !uc->is_visible();
		...
		playlistNum = pm->get_playlist_count() - 1;
		pm->set_active_playlist(playlistNum);
		pm->playlist_execute_default_action(playlistNum, 0);
	...
	else if (cmd == ID_PLAYLIST_CREATE_FOLDER)
	{
		// Create Undo Backup
		if (advconfig_playlist_undo_backup.get())
			pm->playlist_undo_backup(playlistNum);
	...
	// "Show"
	else if (cmd == ID_ACTIVE_PLAYLIST_SHOW || cmd == ID_CURRENT_PLAYLIST_SHOW
		|| cmd >= ID_LISTED_PLAYLIST_SHOW && cmd <= ID_LISTED_PLAYLIST_SHOW_LAST)
	{
		if (cmd == ID_ACTIVE_PLAYLIST_SHOW)
			playlistNum = activePlaylistNum;
		else if (cmd == ID_CURRENT_PLAYLIST_SHOW)
			playlistNum = currentPlaylistNum;
		else
			playlistNum = cmd - ID_LISTED_PLAYLIST_SHOW;
	...
	{
		playlistNum = pm->get_active_playlist();
		pm->remove_playlist_switch(playlistNum);
	...
	// "Add Files..."
	else if (cmd == ID_ACTIVE_PLAYLIST_ADD_FILES || cmd == ID_CURRENT_PLAYLIST_ADD_FILES
		|| cmd >= ID_LISTED_PLAYLIST_ADD_FILES && cmd <= ID_LISTED_PLAYLIST_ADD_FILES_LAST)
	{
		if (cmd == ID_ACTIVE_PLAYLIST_ADD_FILES)
			playlistNum = activePlaylistNum;
		else if (cmd == ID_CURRENT_PLAYLIST_ADD_FILES)
			playlistNum = currentPlaylistNum;
		else
			playlistNum = cmd - ID_LISTED_PLAYLIST_ADD_FILES;
	...
	else if (cmd == ID_ACTIVE_PLAYLIST_ADD_FOLDER || cmd == ID_CURRENT_PLAYLIST_ADD_FOLDER
		|| cmd >= ID_LISTED_PLAYLIST_ADD_FOLDER && cmd <= ID_LISTED_PLAYLIST_ADD_FOLDER_LAST)
	{
		if (cmd == ID_ACTIVE_PLAYLIST_ADD_FOLDER)
			playlistNum = activePlaylistNum;
		else if (cmd == ID_CURRENT_PLAYLIST_ADD_FOLDER)
			playlistNum = currentPlaylistNum;
		else
			playlistNum = cmd - ID_LISTED_PLAYLIST_ADD_FOLDER;
